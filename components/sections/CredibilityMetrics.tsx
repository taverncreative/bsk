...
function AnimatedCounter({ value, prefix = '', suffix = '', decimals = 0 }: Omit<MetricProps, 'label'>) {
  ...
  useEffect(() => {
    if (inView && nodeRef.current && !shouldReduceMotion) {
  ...
  // SSR / initial render: always emit the FINAL value so crawlers (and users with
  // JS disabled) see the real metric. The client-side animation in useEffect will
  // briefly count from 0 → value when the section enters view.
  const finalParts = value.toFixed(decimals).split('.');
  finalParts[0] = finalParts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");

  return (
    <span ref={nodeRef}>
      {prefix}{finalParts.join('.')}{suffix}
    </span>
  );
}
      ...
      >
        {metrics.map((metric, idx) => (
          <div key={idx} className="flex flex-col gap-2 bg-paper-raised border border-[rgba(255,255,255,0.06)] rounded-[18px] p-6 lg:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.6)] items-start md:items-center text-left md:text-center w-full transition-all duration-400 hover:-translate-y-1 hover:border-[rgba(201,162,89,0.3)] group relative overflow-hidden">
            ...
            <span className="text-[15px] xl:text-base font-medium text-ink-muted leading-snug z-10">
              {metric.label}
            </span>
            {metric.subtext && (
