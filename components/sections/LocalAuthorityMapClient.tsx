...
const KENT_BOUNDS = {
  minLat: 50.95, // South coast (Dungeness area)
  maxLat: 51.48, // North coast (Thames estuary)
  minLng: 0.10,  // West (Dartford area)
  maxLng: 1.45,  // East (Thanet / Broadstairs)
};

// Map Kent coordinates to SVG ViewBox (400x300) with padding
function projectCoordinates(lat: number, lng: number) {
  const paddingX = 30;
  const paddingY = 30;
  const usableWidth = 400 - (paddingX * 2);
  const usableHeight = 300 - (paddingY * 2);
  
  const xRatio = (lng - KENT_BOUNDS.minLng) / (KENT_BOUNDS.maxLng - KENT_BOUNDS.minLng);
  // Y is inverted because SVG 0,0 is top-left, but higher latitude is North (Top)
  const yRatio = 1 - ((lat - KENT_BOUNDS.minLat) / (KENT_BOUNDS.maxLat - KENT_BOUNDS.minLat));
  
  // Clamp boundaries just in case to avoid rendering off-map
  const clampedX = Math.max(0, Math.min(1, xRatio));
  const clampedY = Math.max(0, Math.min(1, yRatio));

  return {
    x: paddingX + (clampedX * usableWidth),
    y: paddingY + (clampedY * usableHeight)
  };
}

export default function LocalAuthorityMapClient({ headlineOverride, towns }: LocalAuthorityMapProps) {
